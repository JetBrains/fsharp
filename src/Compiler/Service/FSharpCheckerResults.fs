...
type internal TypeCheckInfo
    ...
    member _.GetDeclarationListSymbols
        (parseResultsOpt, line, lineStr, partialName, isFromAttribute, getAllEntities, genBodyForOverriddenMeth: FSharpCodeCompletionOptions)
        =
        let isSigFile = SourceFileImpl.IsSignatureFile mainInputFileName
...
type FSharpCheckFileResults
    ...
    member _.GetDeclarationListSymbols(parsedFileResults, line, lineText, partialName, isFromAttribute, ?getAllEntities, ?options) =
        let getAllEntities = defaultArg getAllEntities (fun () -> [])
